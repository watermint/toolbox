...
func (z *ExecContext) DefaultWorkPath() string {
	...
	return filepath.Join(u.HomeDir, "."+app.Name)
}
...
func (z *ExecContext) setLogger(logger *zap.Logger) {
	if z.logWrapper == nil {
		z.logWrapper = ut_log.NewLogWrapper(logger)
...
func (z *ExecContext) setupLoggerFile() {
	logPath := filepath.Join(z.LogsPath(), app.Name+".log")
