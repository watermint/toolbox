...
type ZRelocationBatchJobStatus struct {
	dropbox.Tagged

	// Complete : The copy or move batch job has finished.
	Complete json.RawMessage `json:"complete,omitempty"`
	// Failed : The copy or move batch job has failed with exception.
	Failed *files.RelocationBatchError `json:"failed,omitempty"`
}

func (u *ZRelocationBatchJobStatus) UnmarshalJSON(body []byte) error {
	type wrap struct {
		dropbox.Tagged
		// Complete : The copy or move batch job has finished.
		Complete json.RawMessage `json:"complete,omitempty"`
...
func ZMoveBatchCheck(cfg dropbox.Config, arg *async.PollArg) (res *ZRelocationBatchJobStatus, err error) {
	ctx := dropbox.NewContext(cfg)
	dbx := &ctx
	cli := dbx.Client
	...
	if resp.StatusCode == http.StatusConflict {
		var apiError files.MoveBatchCheckAPIError
		err = json.Unmarshal(body, &apiError)
		if err != nil {
			return
		}
