...
```
cd $HOME\Desktop
.\tbx.exe dev test async 
```

macOS, Linux:
```
$HOME/Desktop/tbx dev test async 
```
...
The command will generate a report in three different formats. `rows.csv`, `rows.json`, and `rows.xlsx`.
...
In case of a report become large, a report in `.xlsx` format will be split into several chunks like follows; `rows_0000.xlsx`, `rows_0001.xlsx`, `rows_0002.xlsx`, ...
