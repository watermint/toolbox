...
func verifyTokenTeam(name string, scopes []string, ctx api_auth.OAuthContext, ctl app_control.Control, app api_auth.OAuthApp) (actx api_auth.OAuthContext, err error) {
	l := ctl.Log().With(esl.String("peerName", ctx.PeerName()), esl.Strings("scopes", scopes))
	ui := ctl.UI()
...
func VerifyToken(name string, ctx api_auth.OAuthContext, ctl app_control.Control, app api_auth.OAuthApp) (actx api_auth.OAuthContext, err error) {
	scopes := ctx.Scopes()
	l := ctl.Log().With(esl.String("peerName", ctx.PeerName()), esl.Strings("scopes", scopes))
