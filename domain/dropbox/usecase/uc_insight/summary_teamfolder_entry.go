...
func (z tsImpl) summarizeTeamFolder(teamFolder *TeamFolder, s eq_sequence.Stage) error {
	...
	err := func(tx *gorm.DB) error {
		ne := &NamespaceEntry{}
		if err := tx.First(ne, "namespace_id = ?", teamFolder.TeamFolderId).Error; err != nil {
			l.Debug("Unable to find namespace entry", esl.Error(err))
			return err
		}

		rows, err := tx.Model(&NamespaceEntry{}).Where("parent_folder_id = ?", ne.FileId).Rows()
		if err != nil {
			l.Debug("Unable to find namespace entry", esl.Error(err))
			return err
		}
		...
		for rows.Next() {
			cne := &NamespaceEntry{}
			if err := tx.ScanRows(rows, cne); err != nil {
				l.Debug("Unable to find namespace entry", esl.Error(err))
				return err
			}
...
func (z tsImpl) summarizeTeamFolderEntry(teamFolderEntry *SummarizeTeamFolderEntry, s eq_sequence.Stage) error {
	...
	err := func(tx *gorm.DB) error {
		...
		ne := &NamespaceEntry{}
		if err := tx.First(ne, "file_id = ?", teamFolderEntry.FileId).Error; err != nil {
			l.Debug("Unable to find namespace entry", esl.Error(err))
			return err
		}
		se := &SummaryEntry{}
		if err := tx.First(se, "file_id = ?", teamFolderEntry.FileId).Error; err != nil {
		...
		if ne.EntryType == "folder" {
			...
			for rows.Next() {
				...
				if cne.FileId == "" {
					...
					tx.Save(&SummaryTeamFolderEntry{
						TeamFolderId:       teamFolderEntry.TeamFolder.TeamFolderId,
						FileId:             "",
						Name:               cne.Name,
