...
func (z *Page) download(page *mo_file.NodeWithPath, doc *mo_file.Document, c app_control.Control) error {
	...
	path := filepath.Join(z.Path.Path(), es_filepath.Escape(doc.Name+"-"+page.Node.Name)+"."+z.Format.Value())
...
func (z *Page) Exec(c app_control.Control) error {
	...
	c.UI().Info(z.PageInfo.With("Page", len(pages)))
	var lastErr error
	c.Sequence().Do(func(s eq_sequence.Stage) {
		s.Define("page", z.download, doc, c)
		q := s.Get("page")
