...
func (z *Prepend) Exec(c app_control.Control) error {
	...
	paper, err := sv_paper.New(client).Prepend(z.Path, z.Format.Value(), content)
	if err != nil {
		return err
	}
	z.Created.Row(&paper)
...
func (z *Prepend) Test(c app_control.Control) error {
	...
	return rc_exec.ExecMock(c, &Prepend{}, func(r rc_recipe.Recipe) {
		m := r.(*Prepend)
