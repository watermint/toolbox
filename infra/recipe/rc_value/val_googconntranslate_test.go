...
type ValueGoogConnTranslateRecipe struct {
	Peer goog_conn.ConnGoogleTranslate
}

func (z *ValueGoogConnTranslateRecipe) Preset() {
	z.Peer.SetPeerName("value_test")
}

func (z *ValueGoogConnTranslateRecipe) Exec(c app_control.Control) error {
	return nil
}

func (z *ValueGoogConnTranslateRecipe) Test(c app_control.Control) error {
	return nil
}

func TestValueGoogConnTranslate(t *testing.T) {
	err := qt_control.WithControl(func(c app_control.Control) error {
		rcp0 := &ValueGoogConnTranslateRecipe{}
		...
		// Apply parsed values
		rcp1 := repo.Apply()
		mod1 := rcp1.(*ValueGoogConnTranslateRecipe)
		...
		mod2 := rcp2.(*ValueGoogConnTranslateRecipe)
		if mod1.Peer.PeerName() != "by_argument" {
			t.Error(mod2)
		}
...
func TestValueGoogConnTranslate_Capture(t *testing.T) {
	err := qt_control.WithControl(func(ctl app_control.Control) error {
		v := newValueGoogConnTranslate("123")
		...
		v2 := newValueGoogConnTranslate("123")
