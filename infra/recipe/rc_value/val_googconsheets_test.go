...
type ValueGoogConnSheetsRecipe struct {
	Peer goog_conn.ConnGoogleSheets
}

func (z *ValueGoogConnSheetsRecipe) Preset() {
	z.Peer.SetPeerName("value_test")
}

func (z *ValueGoogConnSheetsRecipe) Exec(c app_control.Control) error {
	return nil
}

func (z *ValueGoogConnSheetsRecipe) Test(c app_control.Control) error {
	return nil
}

func TestValueGoogConnSheets(t *testing.T) {
	err := qt_control.WithControl(func(c app_control.Control) error {
		rcp0 := &ValueGoogConnSheetsRecipe{}
		...
		// Apply parsed values
		rcp1 := repo.Apply()
		mod1 := rcp1.(*ValueGoogConnSheetsRecipe)
		...
		mod2 := rcp2.(*ValueGoogConnSheetsRecipe)
		if mod1.Peer.PeerName() != "by_argument" {
			t.Error(mod2)
		}
...
func TestValueGoogConnSheets_Capture(t *testing.T) {
	err := qt_control.WithControl(func(ctl app_control.Control) error {
		v := newValueGoogConnSheets("123")
