...
type ValueGoogConnCalendarRecipe struct {
	Peer goog_conn.ConnGoogleCalendar
}

func (z *ValueGoogConnCalendarRecipe) Preset() {
	z.Peer.SetPeerName("value_test")
}

func (z *ValueGoogConnCalendarRecipe) Exec(c app_control.Control) error {
	return nil
}

func (z *ValueGoogConnCalendarRecipe) Test(c app_control.Control) error {
	return nil
}

func TestValueGoogConnCalendar(t *testing.T) {
	err := qt_control.WithControl(func(c app_control.Control) error {
		rcp0 := &ValueGoogConnCalendarRecipe{}
		...
		// Apply parsed values
		rcp1 := repo.Apply()
		mod1 := rcp1.(*ValueGoogConnCalendarRecipe)
		...
		mod2 := rcp2.(*ValueGoogConnCalendarRecipe)
		if mod1.Peer.PeerName() != "by_argument" {
			t.Error(mod2)
		}
...
func TestValueGoogConnCalendar_Capture(t *testing.T) {
	err := qt_control.WithControl(func(ctl app_control.Control) error {
		v := newValueGoogConnCalendar("123")
		...
		v2 := newValueGoogConnCalendar("123")
