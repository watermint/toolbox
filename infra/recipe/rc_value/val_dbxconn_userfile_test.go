...
type ValueDbxConnUserFileRecipe struct {
	Peer dbx_conn.ConnUserFile
}

func (z *ValueDbxConnUserFileRecipe) Preset() {
	z.Peer.SetPeerName("value_test")
}

func (z *ValueDbxConnUserFileRecipe) Exec(c app_control.Control) error {
	return nil
}

func (z *ValueDbxConnUserFileRecipe) Test(c app_control.Control) error {
	return nil
}

func TestValueDbxConnUserFile(t *testing.T) {
	err := qt_control.WithControl(func(c app_control.Control) error {
		rcp0 := &ValueDbxConnUserFileRecipe{}
		...
		// Apply parsed values
		rcp1 := repo.Apply()
		mod1 := rcp1.(*ValueDbxConnUserFileRecipe)
		...
		mod2 := rcp2.(*ValueDbxConnUserFileRecipe)
		if mod1.Peer.PeerName() != "by_argument" {
			t.Error(mod2)
		}
...
func TestValueDbxConnUserFile_Capture(t *testing.T) {
	err := qt_control.WithControl(func(ctl app_control.Control) error {
		v := newValueDbxConnUserFile("123")
		...
		v2 := newValueDbxConnUserFile("123")
