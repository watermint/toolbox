...
type ValueDeeeplConnRecipe struct {
	Peer deepl_conn.ConnDeeplApi
}

func (z *ValueDeeeplConnRecipe) Preset() {
	z.Peer.SetPeerName("value_test")
}

func (z *ValueDeeeplConnRecipe) Exec(c app_control.Control) error {
	return nil
}

func (z *ValueDeeeplConnRecipe) Test(c app_control.Control) error {
	return nil
}

func TestValueDeeeplConnRecipe(t *testing.T) {
	err := qt_control.WithControl(func(c app_control.Control) error {
		rcp0 := &ValueDeeeplConnRecipe{}
		...
		// Apply parsed values
		rcp1 := repo.Apply()
		mod1 := rcp1.(*ValueDeeeplConnRecipe)
		...
		mod2 := rcp2.(*ValueDeeeplConnRecipe)
		if mod1.Peer.PeerName() != "by_argument" {
			t.Error(mod2)
		}
...
func TestValueDeeplConn_Capture(t *testing.T) {
	err := qt_control.WithControl(func(ctl app_control.Control) error {
		v := newValueDeeplConn("123")
