---
layout: command
title: コマンド `dropbox file import batch url`
lang: ja
---

# dropbox file import batch url

URLからファイルを一括インポートします (非可逆な操作です)

URLのリストから複数のファイルをダウンロードしてDropboxにインポートします。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file import batch url -file /path/to/data/file -path /path/to/import
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file import batch url -file /path/to/data/file -path /path/to/import
```
