---
layout: command
title: コマンド `dropbox team group clear externalid`
lang: ja
---
...
アイデンティティプロバイダーから移行する場合や統合システムを変更する場合に、グループから外部IDの関連付けを削除します。グループの機能はそのまま残りますが、外部システムのマッピングは失われます。アイデンティティプロバイダーとの同期問題のトラブルシューティングに便利です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group clear externalid -file /PATH/TO/DATA_FILE.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group clear externalid -file /PATH/TO/DATA_FILE.csv
```
