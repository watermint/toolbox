---
layout: command
title: コマンド `dropbox team group rename`
lang: ja
---
...
すべてのメンバーと権限を維持しながら、グループの表示名を更新します。部門が再構築されたり、プロジェクト名が変更されたり、グループの目的が進化したりする場合に便利です。名前の変更は即座に行われ、システム全体でグループへのすべての参照に影響します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group rename -current-name CURRENT_NAME -new-name NEW_NAME
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group rename -current-name CURRENT_NAME -new-name NEW_NAME
```
