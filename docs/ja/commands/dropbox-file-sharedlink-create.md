---
layout: command
title: コマンド `dropbox file sharedlink create`
lang: ja
---
...
オプションのパスワード保護と有効期限でファイルまたはフォルダの共有リンクを作成します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file sharedlink create -path /path/to/share
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file sharedlink create -path /path/to/share
```
...
**-team-only**
: リンクがチームメンバーのみアクセスできます. Default: false
