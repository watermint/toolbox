...
指定したパスのファイルまたはフォルダのメタデータとプロパティを取得します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file info -path /DROPBOX/PATH/TO/FILE
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file info -path /DROPBOX/PATH/TO/FILE
```
