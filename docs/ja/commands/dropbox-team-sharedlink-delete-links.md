---
layout: command
title: コマンド `dropbox team sharedlink delete links`
lang: ja
---
...
経過時間、可視性、またはパスパターンなどの基準に基づいて共有リンクを一括削除します。セキュリティの修復、古いリンクの削除、または新しい共有ポリシーの実施に使用します。削除されたリンクを通じたアクセスを即座に取り消す永続的なアクション。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team sharedlink delete links -file /PATH/TO/DATA_FILE.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team sharedlink delete links -file /PATH/TO/DATA_FILE.csv
```
