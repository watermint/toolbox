---
layout: command
title: コマンド `dropbox team group batch add`
lang: ja
---
...
データファイルからグループを一括作成します。初期設定や組織再編に最適です。部分的な失敗を防ぐため、作成前にすべてのグループを検証します。アイデンティティ管理システムとの統合のために外部IDを含めてください。個別作成よりも大幅に高速です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group batch add -file /PATH/TO/DATA_FILE.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group batch add -file /PATH/TO/DATA_FILE.csv
```
...
**-management-type**
: だれがこのグループを管理できるか (user_managed, company_managed, または system_managed). Options: company_managed (会社の管理者が管理), user_managed (個人ユーザーが管理). Default: company_managed
