---
layout: command
title: コマンド `dropbox team runas file list`
lang: ja
---
...
管理者がメンバーの資格情報なしでメンバーアカウントのファイル一覧を表示できるようにします。問題の調査、コンテンツの監査、またはメンバーがファイルを見つけるのを支援するために不可欠です。すべてのアクションはセキュリティのために記録されます。責任を持って使用し、プライバシーポリシーに従ってください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team runas file list -member-email MEMBER@DOMAIN -path /DROPBOX/PATH/TO/LIST
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team runas file list -member-email MEMBER@DOMAIN -path /DROPBOX/PATH/TO/LIST
```
...
**-include-deleted**
: 削除済みファイルを含める. Default: false

**-include-explicit-shared-members**
:  trueの場合、結果には、各ファイルに明示的なメンバーがいるかどうかを示すフラグが含まれます.. Default: false

**-include-mounted-folders**
:  Trueの場合は、マウントされたフォルダ（appフォルダ、sharedフォルダ、teamフォルダ）のエントリが結果に含まれます.. Default: false
