---
layout: command
title: コマンド `dropbox team teamfolder policy list`
lang: ja
---
...
同期のデフォルト、共有制限、アクセス制御を含むチームフォルダの動作を管理するすべてのポリシーを表示します。フォルダが特定の方法で動作する理由を理解し、ポリシーへの準拠を確保するのに役立ちます。新しいフォルダを作成したり設定を変更したりする前に参照してください。
