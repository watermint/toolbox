---
layout: command
title: コマンド `dropbox team activity daily event`
lang: ja
---

# dropbox team activity daily event

日別アクティビティレポートを生成し、チーム利用パターンとセキュリティ監視に活用 

チームのアクティビティイベントを日別に集計し、チーム行動の傾向や異常を特定しやすくします。日次セキュリティレポートの作成、新機能の採用状況の追跡、セキュリティ上の懸念を示す異常な活動パターンの特定に特に有用です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team activity daily event -start-date DATE
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team activity daily event -start-date DATE
```
