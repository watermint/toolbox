---
layout: command
title: コマンド `dropbox team filerequest list`
lang: ja
---
...
チーム全体のすべてのファイルリクエストの包括的なビュー。外部データ収集の監視、放棄されたリクエストの特定、データ処理ポリシーへの準拠の確保。監査目的でリクエストURL、作成者、提出数を含みます。
...
| 説明                                                                                |
|-------------------------------------------------------------------------------------|
| Dropbox for teams：Dropboxのファイルリクエストを表示                                |
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team filerequest list 
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team filerequest list 
```
