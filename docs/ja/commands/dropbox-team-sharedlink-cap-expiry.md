---
layout: command
title: コマンド `dropbox team sharedlink cap expiry`
lang: ja
---
...
有効期限のない既存の共有リンクに有効期限を適用します。セキュリティコンプライアンスと永続的なリンクの露出削減に不可欠です。経過時間でリンクをターゲットにしたり、一括有効期限ポリシーを適用したりできます。共有コンテンツへの不正な長期アクセスを防ぐのに役立ちます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team sharedlink cap expiry -at "+72h" -file /PATH/TO/shared_link_list.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team sharedlink cap expiry -at "+72h" -file /PATH/TO/shared_link_list.csv
```
