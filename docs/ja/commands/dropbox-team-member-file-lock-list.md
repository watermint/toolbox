---
layout: command
title: コマンド `dropbox team member file lock list`
lang: ja
---

# dropbox team member file lock list

パスの下にあるメンバーのロックを一覧表示 

パス内で特定のメンバーが現在ロックしているすべてのファイルを一覧表示します。コラボレーションのボトルネックの特定、編集の競合のトラブルシューティング、ファイルアクセスパターンの監査に役立ちます。チームメンバーが特定のファイルを編集できない理由を理解するのに便利です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member file lock list -member-email EMAIL -path /DROPBOX/PATH/TO/LIST_LOCK
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member file lock list -member-email EMAIL -path /DROPBOX/PATH/TO/LIST_LOCK
```
...
ロック情報
このコマンドはレポートを3種類の書式で出力します. `lock.csv`, `lock.json`, ならびに `lock.xlsx`.
