---
layout: command
title: コマンド `dropbox team group member add`
lang: ja
---
...
継承された権限と簡素化された管理のためにメンバーをグループに追加します。変更はフォルダアクセスに即座に反映されます。非常に大きなグループの場合は、グループサイズの制限とパフォーマンスへの影響を考慮してください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group member add -group-name GROUP_NAME -member-email EMAIL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group member add -group-name GROUP_NAME -member-email EMAIL
```
