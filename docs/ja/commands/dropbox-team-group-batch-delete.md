---
layout: command
title: コマンド `dropbox team group batch delete`
lang: ja
---
...
複数のグループを単一の操作で効率的に削除します。組織の再構築や古いグループのクリーンアップに便利です。メンバーは個別の権限を保持しますが、グループベースのアクセスを失います。これは元に戻せないため、削除前にグループの内容を確認してください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group batch delete -file /path/to/file.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group batch delete -file /path/to/file.csv
```
