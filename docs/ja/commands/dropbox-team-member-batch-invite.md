---
layout: command
title: コマンド `dropbox team member batch invite`
lang: ja
---
...
CSVファイルから複数のメールアドレスにチーム招待を送信します。SSO環境用のサイレント招待をサポート。新しい部門のオンボーディング、買収、または季節労働者に最適です。送信前にメール形式を検証し、既存のメンバーをチェックします。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member batch invite -file /PATH/TO/member_list.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member batch invite -file /PATH/TO/member_list.csv
```
...
**-silent-invite**
: ウエルカムメールを送信しません (SSOとドメイン確認が必要です). Default: false
