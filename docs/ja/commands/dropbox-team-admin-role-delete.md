---
layout: command
title: コマンド `dropbox team admin role delete`
lang: ja
---

# dropbox team admin role delete

他のロールを保持したままチームメンバーから特定の管理者ロールを削除、正確な権限調整が可能 

他の権限に影響を与えることなく、個々の管理者ロールを選択的に削除します。責任の調整やロールベースのアクセス変更の実装に便利です。コマンドは削除を試みる前にメンバーがロールを持っていることを確認します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team admin role delete -email EMAIL -role-id ROLE_ID
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team admin role delete -email EMAIL -role-id ROLE_ID
```
