---
layout: command
title: コマンド `dropbox team member update batch profile`
lang: ja
---

# dropbox team member update batch profile

メンバーのプロフィール変更 (非可逆な操作です)

名前と姓を含むメンバープロファイル情報を一括更新します。名前形式の標準化、広範なエラーの修正、または組織変更後の更新に最適です。チームディレクトリ全体で一貫性を維持し、検索性を向上させます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member update batch profile -file /path/to/data/file.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member update batch profile -file /path/to/data/file.csv
```
