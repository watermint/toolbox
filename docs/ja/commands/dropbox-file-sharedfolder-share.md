...
既存のフォルダから設定可能な共有ポリシーと権限で共有フォルダを作成します。
...
**-acl-update-policy**
: 共有フォルダーのアクセスコントロールリスト（ACL）を変更できる人.. Options: owner (所有者のみアクセス権限を変更可能), editor (編集者がアクセス権限を変更可能). Default: owner
...
**-shared-link-policy**
: このフォルダー内の共有リンクを閲覧できる人.. Options: anyone (誰でも共有リンクからアクセス可能), members (メンバーのみ共有リンクからアクセス可能). Default: anyone
