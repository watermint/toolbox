---
layout: command
title: コマンド `dropbox file lock all release`
lang: ja
---

# dropbox file lock all release

指定したパスでのすべてのロックを解除する 

現在のユーザーがアカウント全体で保持しているすべてのファイルロックを解放します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file lock all release -path /DROPBOX/PATH/TO/RELEASE/LOCK
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file lock all release -path /DROPBOX/PATH/TO/RELEASE/LOCK
```
