---
layout: command
title: コマンド `dropbox team teamfolder archive`
lang: ja
---

# dropbox team teamfolder archive

チームフォルダのアーカイブ (非可逆な操作です)

アクティブなチームフォルダをアーカイブステータスに変換し、すべてのコンテンツと権限を保持しながら読み取り専用にします。完了したプロジェクト、履歴記録、またはコンプライアンス要件に使用します。アーカイブされたフォルダは必要に応じて再アクティブ化できます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder archive -name チームフォルダ名
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder archive -name チームフォルダ名
```
