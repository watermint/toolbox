---
layout: command
title: コマンド `dropbox team group member delete`
lang: ja
---

# dropbox team group member delete

メンバーをグループから削除 (非可逆な操作です)

他のグループのメンバーシップに影響を与えることなく、単一のグループから個々のメンバーを削除します。対象を絞った権限調整や、メンバーが部門を変更する場合に使用します。削除は即座に有効になり、そのグループから継承されたすべての権限が取り消されます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group member delete -group-name GROUP_NAME -member-email EMAIL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group member delete -group-name GROUP_NAME -member-email EMAIL
```
