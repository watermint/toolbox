---
layout: command
title: コマンド `dropbox file export url`
lang: ja
---
...
共有リンクURLからファイルをダウンロードしてDropboxからエクスポートします。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file export url -local-path /LOCAL/PATH/TO/export -url DOCUMENT_URL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file export url -local-path /LOCAL/PATH/TO/export -url DOCUMENT_URL
```
