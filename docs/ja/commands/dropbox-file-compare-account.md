---
layout: command
title: コマンド `dropbox file compare account`
lang: ja
---
...
二つの異なるDropboxアカウント間でファイルとフォルダを比較して差分を特定します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file compare account -left left -left-path /比較対象パス -right right -right-path /比較対象パス
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file compare account -left left -left-path /比較対象パス -right right -right-path /比較対象パス
```
