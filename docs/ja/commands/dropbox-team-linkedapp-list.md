---
layout: command
title: コマンド `dropbox team linkedapp list`
lang: ja
---
...
チームメンバーのDropboxアカウントにアクセスできるすべてのサードパーティアプリケーションを一覧表示します。セキュリティ監査、不正なアプリの特定、OAuth統合の管理に不可欠です。どのメンバーがどのアプリを使用しているかを表示し、アプリケーションポリシーの実施と潜在的なセキュリティリスクの特定に役立ちます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team linkedapp list 
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team linkedapp list 
```
