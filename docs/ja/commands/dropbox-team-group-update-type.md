---
layout: command
title: コマンド `dropbox team group update type`
lang: ja
---
...
メンバーの追加や削除を誰が行えるかを制御するためにグループ管理設定を変更します。会社管理グループは変更を管理者に制限し、ユーザー管理グループは指定されたメンバーがメンバーシップを管理できるようにします。適切なガバナンスとアクセス制御ポリシーを実装するために重要です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group update type -name グループ名
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group update type -name グループ名
```
...
| `-type`    | グループタイプ（user_managed/company_managed) | company_managed |
...
| group_external_id     | チームがグループに付加することができる外部ID.                                           |
