---
layout: command
title: コマンド `dropbox team group member batch delete`
lang: ja
---

# dropbox team group member batch delete

グループからメンバーを削除 (非可逆な操作です)

CSVファイルマッピングを使用してグループからメンバーを一括削除します。変更を行う前にすべてのメンバーシップを検証します。組織の再構築、オフボーディングプロセス、またはグループメンバーシップのクリーンアップに便利です。問題がある場合は詳細なエラーレポートで効率的に処理します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group member batch delete -file /PATH/TO/DATA_FILE.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group member batch delete -file /PATH/TO/DATA_FILE.csv
```
