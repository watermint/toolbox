---
layout: command
title: コマンド `dropbox file search content`
lang: ja
---
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file search content -query QUERY
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file search content -query QUERY
```
...
**-max-results**
: 返却するエントリーの最大数. Default: 25

**-path**
: 検索対象とするユーザーのDropbox上のパス.
...
このレポートは検索結果とハイライトテキストを出力します.
このコマンドはレポートを3種類の書式で出力します. `matches.csv`, `matches.json`, ならびに `matches.xlsx`.

| 列             | 説明                             |
|----------------|----------------------------------|
| tag            | エントリーの種別                 |
| path_display   | パス                             |
| highlight_html | HTML書式のハイライト済みテキスト |

`-budget-memory low`オプションを指定した場合、レポートはJSON形式のみで生成されます

レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `matches_0000.xlsx`, `matches_0001.xlsx`, `matches_0002.xlsx`, ...
