---
layout: command
title: コマンド `dropbox file tag list`
lang: ja
---
...
Dropboxの特定のファイルまたはフォルダーに関連付けられたすべてのタグを表示します。このコマンドは、コンテンツの整理と分類のために適用されたタグを確認するのに役立ちます。出力には、ファイルパスと共に各タグが表示されます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file tag list -path /DROPBOX/PATH/TO/TARGET
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file tag list -path /DROPBOX/PATH/TO/TARGET
```
