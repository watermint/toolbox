---
layout: command
title: コマンド `dropbox team teamfolder batch archive`
lang: ja
---

# dropbox team teamfolder batch archive

複数のチームフォルダをアーカイブします (非可逆な操作です)

経過時間、名前パターン、またはアクティビティレベルなどの基準に基づいてチームフォルダを一括アーカイブします。フォルダライフサイクル管理を効率化し、整理されたチームスペースの維持に役立ちます。新しい変更を防ぎながらすべてのコンテンツを保持します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder batch archive -file /path/to/file.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder batch archive -file /path/to/file.csv
```
