...
```
cd $HOME\Desktop
.\tbx.exe dev release asset -owner REPOSITORY_OWNER -repo REPOSITORY_NAME -branch BRANCH -path URL_FILE_PATH -text TEXT_CONTENT -message COMMIT_MESSAGE
```

macOS, Linux:

```
$HOME/Desktop/tbx dev release asset -owner REPOSITORY_OWNER -repo REPOSITORY_NAME -branch BRANCH -path URL_FILE_PATH -text TEXT_CONTENT -message COMMIT_MESSAGE
```
...
`commit_0000.xlsx`, `commit_0001.xlsx`, `commit_0002.xlsx`, ...
...
レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます;
`content_0000.xlsx`, `content_0001.xlsx`, `content_0002.xlsx`, ...
