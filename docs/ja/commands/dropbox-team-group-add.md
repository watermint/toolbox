---
layout: command
title: コマンド `dropbox team group add`
lang: ja
---
...
チームメンバーの論理的な編成のためのグループを作成します。グループは一括操作を可能にすることで権限管理を簡素化します。識別しやすい命名規則を検討してください。ガバナンスのニーズに応じて、グループは会社管理またはメンバー管理にできます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group add -name グループ名
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group add -name グループ名
```
...
**-management-type**
: グループ管理タイプ. `company_managed` または `user_managed`. Options: company_managed (会社の管理者が管理), user_managed (個人ユーザーが管理). Default: company_managed
