...
同じDropboxアカウント内でファイルまたはフォルダをある場所から別の場所にコピーします。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file copy -src /SRC/PATH -dst /DST/PATH
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file copy -src /SRC/PATH -dst /DST/PATH
```
