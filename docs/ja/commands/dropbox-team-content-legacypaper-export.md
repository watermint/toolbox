---
layout: command
title: コマンド `dropbox team content legacypaper export`
lang: ja
---

# dropbox team content legacypaper export

チームメンバー全員のPaper文書をローカルパスにエクスポート. 

チームのPaperドキュメントをローカルストレージに一括エクスポートし、移行前またはコンプライアンスアーカイブのためにコンテンツを保存します。HTMLおよびMarkdown形式をサポート。メンバー別の整理されたフォルダ構造を作成します。大量のデータをエクスポートする可能性があるため、利用可能なディスク容量を考慮してください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team content legacypaper export -path /LOCAL/PATH/TO/EXPORT
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team content legacypaper export -path /LOCAL/PATH/TO/EXPORT
```
...
データの書き出し
このコマンドはレポートを3種類の書式で出力します. `paper.csv`, `paper.json`, ならびに `paper.xlsx`.

| 列             | 説明                        |
|----------------|-----------------------------|
| member_email   | メンバーのメールアドレス    |
| paper_doc_id   | PaperドキュメントID         |
| paper_owner    | Paper所有者のメールアドレス |
| paper_title    | Paper タイトル              |
| paper_revision | Paper リビジョン            |
| export_path    | エクスポートファイルのパス  |

`-budget-memory low`オプションを指定した場合、レポートはJSON形式のみで生成されます

レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `paper_0000.xlsx`, `paper_0001.xlsx`, `paper_0002.xlsx`, ...
