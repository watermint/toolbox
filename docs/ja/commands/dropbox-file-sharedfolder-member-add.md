---
layout: command
title: コマンド `dropbox file sharedfolder member add`
lang: ja
---
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file sharedfolder member add -email EMAIL -path /DROPBOX/PATH/TO/ADD
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file sharedfolder member add -email EMAIL -path /DROPBOX/PATH/TO/ADD
```
...
**-access-level**
: アクセス権限 (viewer/editor). Options:.   • editor (編集、コメント、共有が可能).   • viewer (閲覧とコメントが可能).   • viewer_no_comment (閲覧のみ可能). Default: editor
