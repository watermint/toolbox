...
保存されているすべての認証クレデンシャルとその詳細（アプリケーション名、スコープ、ピア名、タイムスタンプを含む）を表示します。アクセスの監査、複数アカウントの管理、認証されているサービスの確認に便利です。
...
```
cd $HOME\Desktop
.\tbx.exe config auth list 
```

macOS, Linux:
```
$HOME/Desktop/tbx config auth list 
```
...
認証クレデンシャルデータ
このコマンドはレポートを3種類の書式で出力します. `entity.csv`, `entity.json`, ならびに `entity.xlsx`.
...
レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `entity_0000.xlsx`, `entity_0001.xlsx`, `entity_0002.xlsx`, ...
