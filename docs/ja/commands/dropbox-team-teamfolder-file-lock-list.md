---
layout: command
title: コマンド `dropbox team teamfolder file lock list`
lang: ja
---
...
ロック保持者情報とロック期間を含むチームフォルダ内の現在ロックされているすべてのファイルを一覧表示します。コラボレーションのボトルネック、古いロック、支援が必要な可能性のあるユーザーを特定するのに役立ちます。スムーズなチームワークフローを維持するために不可欠です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder file lock list -path /DROPBOX/PATH/TO/LIST -team-folder NAME
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder file lock list -path /DROPBOX/PATH/TO/LIST -team-folder NAME
```
