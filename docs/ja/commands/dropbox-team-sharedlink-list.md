---
layout: command
title: コマンド `dropbox team sharedlink list`
lang: ja
---
...
URL、可視性設定、有効期限、作成者を示すすべてのチーム共有リンクの包括的なインベントリ。セキュリティ監査、リスクのあるリンクの特定、外部共有パターンの理解に不可欠です。焦点を絞った分析のためにさまざまな基準でフィルタリングします。
...
**-visibility**
: 可視性によるリンクのフィルタリング (all/public/team_only/password). Options:.   • all (すべての表示オプション).   • public (リンクを知っている人は誰でもアクセス可能).   • team_only (チームメンバーのみアクセス可能).   • password (パスワード保護アクセス).   • team_and_password (チームメンバーのみ、パスワード必須).   • shared_folder_only (共有フォルダーメンバーのみアクセス可能). Default: all
