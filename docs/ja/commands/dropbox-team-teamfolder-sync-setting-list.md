...
すべてのチームフォルダの現在の同期設定を表示し、新しいメンバーのデバイスに自動的に同期するかどうかを示します。帯域幅への影響、ストレージ要件を理解し、適切なコンテンツ配布ポリシーを確保するのに役立ちます。
...
**-scan-all**
: すべての深さのスキャンを実行する（フォルダ構造によってはかなりの時間がかかる場合があります）. Default: false

**-show-all**
: スキャンしたフォルダーをすべて表示する. Default: false
