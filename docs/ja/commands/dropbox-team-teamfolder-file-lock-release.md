---
layout: command
title: コマンド `dropbox team teamfolder file lock release`
lang: ja
---
...
特定のファイルが作業をブロックしている場合に、チームフォルダ内の個々のファイルロックを解除します。特定のファイルのみのロック解除が必要な場合、一括解除よりも精密です。他のロックされたファイルに影響を与えることなく、緊急の編集の競合を解決するのに便利です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder file lock release -path /DROPBOX/PATH/TO/RELEASE -team-folder NAME
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder file lock release -path /DROPBOX/PATH/TO/RELEASE -team-folder NAME
```
