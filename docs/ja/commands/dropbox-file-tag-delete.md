---
layout: command
title: コマンド `dropbox file tag delete`
lang: ja
---
...
Dropboxのファイルまたはフォルダーから特定のタグを削除します。この操作はタグの関連付けのみを削除し、ファイルまたはフォルダー自体には影響しません。古いまたは不正確なタグをクリーンアップするためにこのコマンドを使用してください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file tag delete -path /DROPBOX/PATH/TO/TARGET -tag TAG_NAME
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file tag delete -path /DROPBOX/PATH/TO/TARGET -tag TAG_NAME
```
