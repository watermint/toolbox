---
layout: command
title: コマンド `dropbox team runas sharedfolder mount mountable`
lang: ja
---

# dropbox team runas sharedfolder mount mountable

メンバーがマウントできるすべての共有フォルダーをリストアップ. 

メンバーがアクセスできるが、現在デバイスに同期されていない共有フォルダを一覧表示します。利用可能なフォルダの特定、メンバーがコンテンツを見つけるのを支援、または特定のフォルダがローカルに表示されない理由を理解するのに便利です。潜在的な同期オプションを表示します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team runas sharedfolder mount mountable -member-email EMAIL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team runas sharedfolder mount mountable -member-email EMAIL
```
