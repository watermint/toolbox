---
layout: command
title: コマンド `dropbox team teamfolder permdelete`
lang: ja
---

# dropbox team teamfolder permdelete

チームフォルダを完全に削除します (非可逆な操作です)

チームフォルダとすべての含まれるファイルを不可逆的に削除します。重要なデータが残っていないことを確認した後、適切な承認を得てのみ使用してください。データ保持ポリシーへの準拠または機密コンテンツの削除に不可欠です。このアクションは元に戻すことができません。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder permdelete -name チームフォルダ名
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder permdelete -name チームフォルダ名
```
