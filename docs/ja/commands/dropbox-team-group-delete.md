---
layout: command
title: コマンド `dropbox team group delete`
lang: ja
---
...
グループを完全に削除し、すべてのメンバーの関連付けを削除します。メンバーは他のグループまたは個別の権限を通じてアクセスを保持します。元に戻すことはできません - 不確かな場合は、代わりにメンバーを削除してグループをアーカイブすることを検討してください。このグループを使用するフォルダ権限も削除されます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group delete -name グループ名
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group delete -name グループ名
```
