---
layout: command
title: コマンド `dropbox team teamfolder add`
lang: ja
---

# dropbox team teamfolder add

集約されたチームコンテンツストレージとコラボレーション用の新しいチームフォルダを作成 (非可逆な操作です)

集中化されたチームコンテンツストレージとコラボレーション用の新しいチームフォルダを作成します。プロジェクトベースの作業、部門別フォルダ、共有リソースに最適です。作成時に同期設定（デフォルトまたは非同期）を指定できます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder add -name NAME
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder add -name NAME
```
...
**-sync-setting**
: チームフォルダの同期設定. Options: default (デフォルト同期設定), not_synced (同期しない). Default: default
...
このコマンドはレポートを3種類の書式で出力します. `added.csv`, `added.json`, ならびに `added.xlsx`.
...
レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `added_0000.xlsx`, `added_0001.xlsx`, `added_0002.xlsx`, ...
