---
layout: command
title: コマンド `dropbox team member update batch externalid`
lang: ja
---
...
外部アイデンティティシステムIDをDropboxチームメンバーに一括マッピングします。SCIM統合、SSOセットアップ、またはHRシステムとの同期に重要です。プラットフォーム間で一貫したアイデンティティマッピングを確保します。必要に応じて既存のIDを更新するか、新しいIDを設定します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member update batch externalid -file /path/to/file.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member update batch externalid -file /path/to/file.csv
```
