---
layout: command
title: コマンド `dropbox team runas sharedfolder batch leave`
lang: ja
---
...
メンバーの操作なしに複数の共有フォルダからメンバーを削除する管理者ツール。アクセスのクリーンアップ、セキュリティ対応、または組織変更に便利です。適切な監査証跡を維持しながら、メンバーとして動作します。フォルダ所有者は削除できません。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team runas sharedfolder batch leave -file /PATH/TO/DATA_FILE.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team runas sharedfolder batch leave -file /PATH/TO/DATA_FILE.csv
```
