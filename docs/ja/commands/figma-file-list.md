...
```
cd $HOME\Desktop
.\tbx.exe figma file list -project-id PROJECT_ID. チーム内のPROJECT_IDを取得するには、`services figma project list`コマンドを使用します。
```

macOS, Linux:
```
$HOME/Desktop/tbx figma file list -project-id PROJECT_ID. チーム内のPROJECT_IDを取得するには、`services figma project list`コマンドを使用します。
```
...
レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `files_0000.xlsx`, `files_0001.xlsx`, `files_0002.xlsx`, ...
