---
layout: command
title: コマンド `dropbox file sharedfolder mount delete`
lang: ja
---
...
成功すると、現在のユーザーは、そのフォルダーを再度追加しない限り、そのフォルダーにアクセスできなくなります. `dropbox file sharedfolder mount list`コマンドを使って、削除したいフォルダのshared_folder_idを見つけてください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file sharedfolder mount delete -shared-folder-id SHARED_FOLDER_ID
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file sharedfolder mount delete -shared-folder-id SHARED_FOLDER_ID
```
