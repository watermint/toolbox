---
layout: command
title: コマンド `dropbox team sharedlink update visibility`
lang: ja
---
...
共有リンクの可視性をパブリックからチームのみまたは他の制限された設定に更新します。外部への露出を減らし、コンプライアンス要件を満たすために不可欠です。現在の可視性レベルまたはコンテンツの場所でリンクをターゲットにできます。変更は即座に有効になります。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team sharedlink update visibility -file /PATH/TO/DATA_FILE.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team sharedlink update visibility -file /PATH/TO/DATA_FILE.csv
```
...
**-new-visibility**
: 新しい視認性設定. Options: public (公開設定), team_only (チームのみ設定). Default: team_only
