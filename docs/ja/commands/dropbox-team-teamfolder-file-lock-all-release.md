---
layout: command
title: コマンド `dropbox team teamfolder file lock all release`
lang: ja
---

# dropbox team teamfolder file lock all release

チームフォルダのパスの下にあるすべてのロックを解除する 

指定されたチームフォルダ内のすべてのファイルロックを一括解除します。複数のロックがチームの生産性をブロックしている場合やシステムの問題の後に使用します。可能な場合はロック保持者に通知します。ロックされたファイルの未保存の変更が失われる可能性があります。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder file lock all release -path /DROPBOX/PATH/TO/RELEASE -team-folder NAME
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder file lock all release -path /DROPBOX/PATH/TO/RELEASE -team-folder NAME
```
