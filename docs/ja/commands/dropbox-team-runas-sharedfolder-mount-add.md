---
layout: command
title: コマンド `dropbox team runas sharedfolder mount add`
lang: ja
---
...
メンバーが自分でできない場合に、メンバーアカウントに共有フォルダをマウントする管理者アクション。同期の問題のトラブルシューティング、技術的でないユーザーの支援、または重要なフォルダが適切にマウントされていることを確認するのに便利です。メンバーがアクションを実行したかのように動作します。
