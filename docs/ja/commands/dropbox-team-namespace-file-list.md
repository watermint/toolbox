...
フィルタリングオプションを使用してチームネームスペース内のすべてのファイルとフォルダを一覧表示します。削除されたアイテム、メンバーフォルダ、共有フォルダ、チームフォルダを含めるか除外します。コンテンツ監査、移行計画、ネームスペースタイプ全体のデータ分布の理解に不可欠です。
...
**-include-deleted**
: 指定された場合、削除済みのファイルやフォルダが含めます. Default: false

**-include-member-folder**
: 指定された場合、チームメンバーのフォルダを含めます. Default: false
