---
layout: command
title: コマンド `dropbox team member file lock all release`
lang: ja
---
...
指定されたフォルダパス内でメンバーが保持しているすべてのファイルロックを一括解除します。メンバーが予期せず退社した場合やシステムの問題が発生した場合に不可欠です。効率性のためにバッチで処理します。ロックされたファイルの未保存の変更が失われる可能性があるため、影響を受けるユーザーへの通知を検討してください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member file lock all release -member-email VALUE -path /DROPBOX/PATH/TO/RELEASE/LOCK
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member file lock all release -member-email VALUE -path /DROPBOX/PATH/TO/RELEASE/LOCK
```
