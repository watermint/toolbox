---
layout: command
title: コマンド `util text nlp english token`
lang: ja
---
...
```
cd $HOME\Desktop
.\tbx.exe util text nlp english token -in /LOCAL/PATH/TO/INPUT.txt
```

macOS, Linux:
```
$HOME/Desktop/tbx util text nlp english token -in /LOCAL/PATH/TO/INPUT.txt
```
...
**-ignore-line-break**
: トークン化時に改行を通常の空白とみなす. Default: false
