---
layout: command
title: コマンド `dropbox team member batch detach`
lang: ja
---
...
チームメンバーを個人のDropbox Basicアカウントに一括変換します。メンバーはファイルを保持しますが、チーム機能と共有フォルダへのアクセスを失います。契約終了の請負業者やチームの縮小時に便利です。デタッチする前にデータ保持ポリシーを検討してください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member batch detach -file /PATH/TO/member_list.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member batch detach -file /PATH/TO/member_list.csv
```
...
**-revoke-team-shares**
: 指定した場合にはユーザーからチームが保有する共有フォルダからのアクセス権を剥奪します. Default: false
