---
layout: command
title: コマンド `dropbox file template capture`
lang: ja
---
...
Dropboxパスからファイル/フォルダ構造をキャプチャして再利用可能なテンプレートとして保存します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file template capture -out /LOCAL/PATH/template.json -path /DROPBOX/PATH/TO/CAPTURE
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file template capture -out /LOCAL/PATH/template.json -path /DROPBOX/PATH/TO/CAPTURE
```
