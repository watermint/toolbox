---
layout: command
title: コマンド `dropbox team member file lock release`
lang: ja
---
...
メンバーが保持している単一のファイルロックを解除し、他のユーザーが編集できるようにします。特定のファイルがチームのコラボレーションをブロックしている場合や、ロック保持者が利用できない場合に使用します。特定のファイルのみのロック解除が必要な場合、一括解除よりも精密です。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member file lock release -member-email VALUE -path /DROPBOX/PATH/TO/RELEASE/LOCK
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member file lock release -member-email VALUE -path /DROPBOX/PATH/TO/RELEASE/LOCK
```
