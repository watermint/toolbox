---
layout: command
title: コマンド `dropbox team teamfolder batch permdelete`
lang: ja
---
...
複数のチームフォルダとそのすべてのコンテンツを回復の可能性なしに永久に削除します。古いデータの削除、保持ポリシーへの準拠、または緊急クリーンアップのために適切な承認を得てのみ使用してください。このアクションは元に戻すことができません。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team teamfolder batch permdelete -file /path/to/file.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team teamfolder batch permdelete -file /path/to/file.csv
```
