---
layout: command
title: コマンド `dropbox file request list`
lang: ja
---
...
アカウント内のすべてのファイルリクエストをステータスと詳細とともにリスト表示します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file request list 
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file request list 
```
...
このコマンドはレポートを3種類の書式で出力します. `file_requests.csv`, `file_requests.json`, ならびに `file_requests.xlsx`.
...
レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `file_requests_0000.xlsx`, `file_requests_0001.xlsx`, `file_requests_0002.xlsx`, ...
