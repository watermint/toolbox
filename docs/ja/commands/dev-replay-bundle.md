...
**-peer-name**
: 配置用アカウントのエイリアス. Default: deploy

**-replay-path**
: リプレイのリポジトリパス指定されていない場合は環境変数 `TOOLBOX_REPLAY_PATH` にフォールバックします.

**-results-path**
: テストの失敗結果の送信先パス. Default: /watermint-toolbox-logs/{% raw %}{{.{% endraw %}Date}}-{% raw %}{{.{% endraw %}Time}}/{% raw %}{{.{% endraw %}Random}}

**-timeout**
: テスト結果のアップロード操作のタイムアウト. Default: 60
