---
layout: command
title: コマンド `dropbox team sharedlink delete member`
lang: ja
---
...
コンテンツの場所に関係なく、特定のメンバーが作成したすべての共有リンクを削除します。安全なオフボーディング、侵害されたアカウントへの対応、または即座のアクセス取り消しの実施に不可欠です。元に戻すことはできないため、適切な承認を得て使用してください。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team sharedlink delete member -member-email EMAIL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team sharedlink delete member -member-email EMAIL
```
