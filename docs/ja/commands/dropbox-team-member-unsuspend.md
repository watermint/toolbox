---
layout: command
title: コマンド `dropbox team member unsuspend`
lang: ja
---
...
停止されたメンバーのアカウントを再アクティブ化し、データとチームリソースへの完全なアクセスを復元します。以前のすべての権限、グループメンバーシップ、および設定が保持されます。停止理由が解決されたり、メンバーが休暇から戻ったりする場合に使用します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member unsuspend -email EMAIL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member unsuspend -email EMAIL
```
