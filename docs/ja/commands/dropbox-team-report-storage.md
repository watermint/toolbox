---
layout: command
title: コマンド `dropbox team report storage`
lang: ja
---
...
チームのストレージ消費、使用傾向、メンバー別分布を示す詳細なストレージ使用レポートを作成します。容量計画、コスト管理、使用量の最適化に重要です。ストレージクォータの調整や追加容量の必要性を特定するのに役立ちます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team report storage 
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team report storage 
```
