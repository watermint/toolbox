---
layout: command
title: コマンド `dropbox team content legacypaper count`
lang: ja
---

# dropbox team content legacypaper count

メンバー1人あたりのPaper文書の枚数 

メンバーごとのPaperドキュメント数を提供し、作成されたドキュメントとアクセスされたドキュメントを区別します。PaperからDropboxへの移行計画、ヘビーユーザーの特定、移行範囲の見積もりに不可欠です。フィルタオプションにより関連するドキュメントセットに焦点を当てることができます。
...
Paper文書数のカウント
このコマンドはレポートを3種類の書式で出力します. `stats.csv`, `stats.json`, ならびに `stats.xlsx`.

| 列           | 説明                      |
|--------------|---------------------------|
| member_email | メンバーのメールアドレス  |
| created      | 作成されたPaper文書の数   |
| accessed     | アクセスしたPaper文書の数 |

`-budget-memory low`オプションを指定した場合、レポートはJSON形式のみで生成されます

レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `stats_0000.xlsx`, `stats_0001.xlsx`, `stats_0002.xlsx`, ...
