---
layout: command
title: コマンド `dropbox team admin role add`
lang: ja
---

# dropbox team admin role add

個々のチームメンバーに特定の管理者ロールを付与、きめ細かな権限管理を実現 

個々のメンバーに特定の管理者ロールを割り当て、正確な権限制御を行います。チームメンバーを管理職に昇進させたり、責任を調整したりする際に使用します。コマンドは、重複を防ぐためにメンバーが指定されたロールをまだ持っていないことを検証します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team admin role add -email EMAIL -role-id ROLE_ID
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team admin role add -email EMAIL -role-id ROLE_ID
```
