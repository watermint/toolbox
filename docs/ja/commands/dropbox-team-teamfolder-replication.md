---
layout: command
title: コマンド `dropbox team teamfolder replication`
lang: ja
---

# dropbox team teamfolder replication

チームフォルダを他のチームに複製します (試験的実装かつ非可逆な操作です)

構造、権限、コンテンツを保持してチームフォルダの正確な複製を作成します。バックアップの作成、テスト環境の設定、または大きな変更の準備に使用します。大きなフォルダの場合は利用可能なストレージとレプリケーション時間を考慮してください。
...
**-dst-peer-name**
: 宛先チームのアカウント別名. Default: dst

**-name**
: チームフォルダ名

**-src-peer-name**
: 元チームのアカウント別名. Default: src
...
このコマンドはレポートを3種類の書式で出力します. `verification.csv`, `verification.json`, ならびに `verification.xlsx`.
...
レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `verification_0000.xlsx`, `verification_0001.xlsx`, `verification_0002.xlsx`, ...
