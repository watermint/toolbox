---
layout: command
title: コマンド `dropbox team member batch reinvite`
lang: ja
---

# dropbox team member batch reinvite

招待済み状態メンバーをチームに再招待します (非可逆な操作です)

保留中のステータスを持つすべてのメンバーに招待を再送信します。初回の招待が期限切れになったり、スパムで失われたり、メール配信の問題を解決した後に便利です。SSO環境では無音で送信できます。完全なチームオンボーディングを確保するのに役立ちます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member batch reinvite 
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member batch reinvite 
```
...
**-silent**
: 招待メールを送信しません (SSOが必須となります). Default: false
