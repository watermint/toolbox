---
layout: command
title: コマンド `dropbox team member batch suspend`
lang: ja
---
...
チームメンバーを一括停止し、すべてのデータと設定を保持しながらアクセスをブロックします。長期休暇、セキュリティ調査、または一時的なアクセス制限に使用します。デバイスからデータを保持または削除するオプション。メンバーは後で完全なアクセスを復元して停止解除できます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member batch suspend -file /PATH/TO/member_list.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member batch suspend -file /PATH/TO/member_list.csv
```
...
**-keep-data**
: リンク先のデバイスにユーザーのデータを保持する. Default: false
