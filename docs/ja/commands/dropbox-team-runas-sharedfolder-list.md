---
layout: command
title: コマンド `dropbox team runas sharedfolder list`
lang: ja
---

# dropbox team runas sharedfolder list

共有フォルダーの一覧をメンバーとして実行 

権限レベルとフォルダの詳細を含むメンバーの共有フォルダアクセスの管理者ビュー。アクセス監査、過剰共有の調査、または権限の問題のトラブルシューティングに不可欠です。適切なアクセスレベルを確保し、セキュリティリスクを特定するのに役立ちます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team runas sharedfolder list -member-email EMAIL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team runas sharedfolder list -member-email EMAIL
```
...
このコマンドはレポートを3種類の書式で出力します. `shared_folder.csv`, `shared_folder.json`, ならびに `shared_folder.xlsx`.
...
レポートが大きなものとなる場合、`.xlsx`フォーマットのファイルは次のようにいくつかに分割されて出力されます; `shared_folder_0000.xlsx`, `shared_folder_0001.xlsx`, `shared_folder_0002.xlsx`, ...
