---
layout: command
title: コマンド `google sheets sheet list`
lang: ja
---
...
```
cd $HOME\Desktop
.\tbx.exe google sheets sheet list -id GOOGLE_SPREADSHEET_ID
```

macOS, Linux:
```
$HOME/Desktop/tbx google sheets sheet list -id GOOGLE_SPREADSHEET_ID
```
...
シートデータ
このコマンドはレポートを3種類の書式で出力します. `sheets.csv`, `sheets.json`, ならびに `sheets.xlsx`.
