---
layout: command
title: コマンド `dropbox team group member batch add`
lang: ja
---
...
マッピングファイルを使用してメンバーをグループに一括追加します。変更を適用する前にすべてのメンバーシップを検証します。オンボーディング、部門変更、または権限標準化プロジェクトに最適です。詳細なレポートでエラーを適切に処理します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team group member batch add -file /PATH/TO/DATA_FILE.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team group member batch add -file /PATH/TO/DATA_FILE.csv
```
