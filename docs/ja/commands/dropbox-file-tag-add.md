---
layout: command
title: コマンド `dropbox file tag add`
lang: ja
---
...
Dropboxのファイルまたはフォルダーにカスタムタグを追加します。タグはコンテンツの整理と分類に役立ち、検索と管理を容易にします。同じファイルまたはフォルダーに複数のタグを追加できます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file tag add -path /DROPBOX/PATH/TO/TARGET -tag TAG_NAME
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file tag add -path /DROPBOX/PATH/TO/TARGET -tag TAG_NAME
```
