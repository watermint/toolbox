---
layout: command
title: コマンド `dropbox team member batch delete`
lang: ja
---
...
転送を通じてデータを保持しながら、チームメンバーを一括削除します。ファイル転送の宛先メンバーと管理者通知メールの指定が必要です。レイオフ、部門閉鎖、または大量のオフボーディングに最適です。セキュリティのためにオプションでリンクされたデバイスからデータを消去します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member batch delete -file /PATH/TO/member_list.csv
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member batch delete -file /PATH/TO/member_list.csv
```
...
| `-transfer-dest-member`                 | 指定された場合は、指定ユーザーに削除するメンバーのコンテンツを転送します.       |            |
| `-transfer-notify-admin-email-on-error` | 指定された場合は、転送時にエラーが発生した時にこのユーザーにメールを送信します. |            |
