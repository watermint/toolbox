---
layout: command
title: コマンド `dropbox team member suspend`
lang: ja
---
...
すべてのデータ、設定、グループメンバーシップを維持しながら、メンバーアクセスを即座にブロックします。セキュリティインシデント、ポリシー違反、または一時的な休暇に使用します。リンクされたデバイスにデータを保持するかどうかを選択します。メンバーは後で完全なアクセスを復元して停止解除できます。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member suspend -email EMAIL
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member suspend -email EMAIL
```
