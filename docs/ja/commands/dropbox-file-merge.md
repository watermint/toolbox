...
あるフォルダの内容を別のフォルダにマージし、ドライランと空フォルダの処理オプションを提供します。
...
```
cd $HOME\Desktop
.\tbx.exe dropbox file merge -from /from/path -to /path/to
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox file merge -from /from/path -to /path/to
```
...
**-dry-run**
: リハーサルを行います. Default: true

**-from**
: 統合するパス

**-keep-empty-folder**
: 統合後に空となったフォルダを維持する. Default: false
...
: ネームスペースを超えないように制御します. これは共有リンクを含む共有権限設定を維持するためです.. Default: false
