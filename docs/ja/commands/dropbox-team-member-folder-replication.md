---
layout: command
title: コマンド `dropbox team member folder replication`
lang: ja
---
...
このコマンドは、コピー元フォルダ以下のファイルをコピー先フォルダに複製します. ソース フォルダは、ソース メンバーの個人フォルダ、共有フォルダ、またはチーム フォルダのいずれかになります. しかし、それはマウントされ、アクセス可能でなければなりません. このコマンドは、保存先のパスに既にファイルが存在する場合に、ファイルを上書きします.
...
```
cd $HOME\Desktop
.\tbx.exe dropbox team member folder replication -dst-path /コピー先パス -src-path /コピー元パス -dst-member-email コピー先メンバーのメールアドレス -src-member-email コピー元メンバーのメールアドレス
```

macOS, Linux:
```
$HOME/Desktop/tbx dropbox team member folder replication -dst-path /コピー先パス -src-path /コピー元パス -dst-member-email コピー先メンバーのメールアドレス -src-member-email コピー元メンバーのメールアドレス
```
...
**-dst-path**
: コピー先チームメンバーのパス. ルート (/) パスは許可されていないことに注意してください. ルート以下の任意のフォルダを選択する必要があります.
