---
layout: command
title: Command `dev doc review options`
lang: en
---
...
```
cd $HOME\Desktop
.\tbx.exe dev doc review options 
```

macOS, Linux:
```
$HOME/Desktop/tbx dev doc review options 
```
...
## Report: missing_options

Missing SelectString option
The command will generate a report in three different formats. `missing_options.csv`, `missing_options.json`, and `missing_options.xlsx`.
...
In case of a report becomes large, a report in `.xlsx` format will be split into several chunks like follows; `missing_options_0000.xlsx`, `missing_options_0001.xlsx`, `missing_options_0002.xlsx`, ...
