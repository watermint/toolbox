...
func (z OutcomeBase) IfError(f func() es_idiom_deprecated.Outcome) es_idiom_deprecated.Outcome {
	if z.Err != nil {
		return f()
	}
	return z
...
func AssertOutcomeSuccess(outcome es_idiom_deprecated.Outcome) bool {
	if outcome.IsError() {
		return false
	}
	if !outcome.IsOk() {
	...
	ifErr := false
	outcome.IfError(func() es_idiom_deprecated.Outcome {
		ifErr = true
		return outcome
	})
	if ifErr {
...
func AssertOutcomeFailure(outcome es_idiom_deprecated.Outcome) bool {
	if !outcome.IsError() {
		return false
	}
	if outcome.IsOk() {
