...
func (z *Package) createPackage(c app_control.Control) (path string, err error) {
	...
	name := fmt.Sprintf("%s-%s-%s.zip", app.ExecutableName, app.Version, platform)
	...
	// binary
	{
		...
		if ok {
			...
			case "windows/amd64":
				binarySuffix = ".exe"
				binaryName = app.ExecutableName + "-windows-amd64.exe"
			case "linux/amd64":
				binaryName = app.ExecutableName + "-linux-amd64"
			case "linux/arm64":
				binaryName = app.ExecutableName + "-linux-arm64"
			case "darwin/amd64":
				binaryName = app.ExecutableName + "-darwin-amd64"
			case "darwin/arm64":
				binaryName = app.ExecutableName + "-darwin-arm64"
			}
		}
		binaryPath := filepath.Join(z.BuildPath.Path(), binaryName)
		newBinaryPath := filepath.Join(z.BuildPath.Path(), app.ExecutableName+binarySuffix)
...
func (z *Package) Exec(c app_control.Control) error {
	...
	if z.DeployPath.IsExists() {
		...
		return rc_exec.Exec(c, z.Up, func(r rc_recipe.Recipe) {
			...
			m.DropboxPath = mo_path2.NewDropboxPath(filepath.ToSlash(filepath.Join(z.DeployPath.Value(), app.BuildInfo.Branch, app.ExecutableName+"-"+app.BuildId)))
			m.PeerName = "deploy"
		})
	}
