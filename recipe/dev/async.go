...
func (z *AsyncWorker) Exec() error {
	l := z.ctl.Log()
	l.Debug("Scan group (Multi thread)", es_log.String("Routine", ut_runtime.GetGoRoutineName()), es_log.Any("Group", z.group))
...
func (z *Async) Exec(c app_control.Control) error {
	...
	for _, group := range groups {
		if z.RunConcurrently {
			...
			c.Log().Debug("Scan group (Single thread)", es_log.String("Routine", ut_runtime.GetGoRoutineName()), es_log.Any("Group", group))
