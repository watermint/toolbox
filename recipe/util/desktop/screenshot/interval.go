...
import (
	...
	"github.com/watermint/toolbox/domain/desktop/dd_screenshot"
...
func (z *Interval) Exec(c app_control.Control) error {
	l := c.Log()
	if err := dd_screenshot.CheckDisplayAvailability(z.DisplayId.Value()); err != nil {
	...
	for {
		seq++
		name, err := dd_screenshot.SnapName(nameTmpl, z.DisplayId.Value(), seq, bounds)
		if err != nil {
...
func (z *Interval) Test(c app_control.Control) error {
	...
	switch {
	case errors.Is(err, dd_screenshot.ErrorNoDisplay), err == nil:
		return nil
	default:
		return err
	}
