...
func (z *Overwrite) Exec(c app_control.Control) error {
	...
	paper, err := sv_paper.New(z.Peer.Context()).Overwrite(z.Path, z.Format.Value(), content)
	if err != nil {
		return err
	}
	z.Created.Row(&paper)
...
func (z *Overwrite) Test(c app_control.Control) error {
	...
	return rc_exec.ExecMock(c, &Overwrite{}, func(r rc_recipe.Recipe) {
		m := r.(*Overwrite)
