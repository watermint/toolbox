...
func (z *List) Exec(c app_control.Control) error {
	...
	isMemberFolder := func(f mo_file.Entry) bool {
		for _, m := range members {
			if f.Concrete().SharedFolderId == m.MemberFolderId {
				return true
			}
		}
		return false
	...
	allTopFolders, err := sv_file.NewFiles(client).List(mo_path.NewDropboxPath("/"))
	if err != nil {
		return err
	}

	svsf := sv_sharedfolder.New(z.Peer.Client().AsAdminId(admin.TeamMemberId))
	topFolders := make([]*mo_sharedfolder.SharedFolder, 0)
	for _, f := range allTopFolders {
		if isMemberFolder(f) {
			continue
		}
		// shared_folder_id is empty when the folder is the authenticated admin's home folder.
		if f.Concrete().SharedFolderId == "" {
			continue
		}

		sf, err := svsf.Resolve(f.Concrete().SharedFolderId)
		if err != nil {
			return err
		}
		topFolders = append(topFolders, sf)
	}
