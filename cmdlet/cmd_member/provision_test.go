...
func TestCmdMemberProvisioning(t *testing.T) {
	...
	cmdlet.CmdTest(t, NewCmdMember(), []string{"list", "-report-path", memberListPath})
	...
	for {
		...
		if strings.HasSuffix(member.Profile.Email, "@example.com") {
			cmdlet.CmdTest(t, NewCmdMember(), []string{
				"remove",
				"-keep-account=false",
				"-wipe-data=true",
	...
	cmdlet.CmdTestWithTimeout(t, NewCmdMember(), []string{
		"sync",
		"-report-path",
		memberSyncResult,
		"-csv",
